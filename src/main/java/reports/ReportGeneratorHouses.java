...
public class ReportGeneratorHouses {
    private static final String DB_URL = "jdbc:sqlite:real_estate.db";

    public static void generateReport() {
        try {
            InputStream reportStream = ReportGeneratorHouses.class.getResourceAsStream("/Viviendas.jasper");

            Connection connection = DriverManager.getConnection(DB_URL);

            Map<String, Object> parameters = new HashMap<>();
            parameters.put("ReportTitle", "Lista de Viviendas");

            JasperPrint jasperPrint = JasperFillManager.fillReport(reportStream, parameters, connection);

            String pdfFilePath = "reporte_viviendas.pdf";
